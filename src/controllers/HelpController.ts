...
class HelpController {
  async sendGetHelpData(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!reportedTripId || !reporterId || !reportedId || !complaint)
        return Respond.error(
          res,
          'reportedTripId, reporterId, reportedId, issueMessage fields are required'
        )

      const helpData = await recordGetHelpData({
        reportedTripId,
        reporterId,
        reportedId,
        complaint,
